...
//        os_printf(precvbuffer);
        ...
        switch (pURL_Frame->Type) {
            case GET:
                ...
                int pinNum;
                int len;
                if (os_strcmp(pURL_Frame->pSelect, "mcu") == 0 ){
                    if (os_strcmp(pURL_Frame->pCommand, "data") == 0){
                        for(len=0;len<strlen(pURL_Frame->pFilename);len++)
                        {
                            uart_tx_one_char_no_wait(0,pURL_Frame->pFilename[len]);
                        }
                    }
                    char resp[64];
                    os_sprintf(resp,"send=%s ok!",pURL_Frame->pFilename);
                    data_send(ptrespconn, true, resp );
                }else if (os_strcmp(pURL_Frame->pSelect, "pin") == 0 ){
                    if (os_strcmp(pURL_Frame->pCommand, "get") == 0){
                        pinNum = atoi(pURL_Frame->pFilename);
                    }else if (os_strcmp(pURL_Frame->pCommand, "on") == 0){
                        pinNum = atoi(pURL_Frame->pFilename);
                        os_printf("set pinNum=%d\n");
                        GPIO_OUTPUT_SET(pinNum, 1);

                    }else if (os_strcmp(pURL_Frame->pCommand, "off") == 0){
                        pinNum = atoi(pURL_Frame->pFilename);
                        os_printf("set pinNum=%d\n");
                        GPIO_OUTPUT_SET(pinNum, 0);
                    }

                    char stats=GPIO_INPUT_GET(pinNum);
                    char resp[64];
                    os_sprintf(resp,"pin[%d]=%c",pinNum,stats+'0');
                    data_send(ptrespconn, true, resp );
            ...
            case POST:
                ...
                if (pParseBuffer == NULL) {
                    SyslogSendString("pParseBuffer == NULL\n");
                    break;
                }
                ...
                if (os_strcmp(pURL_Frame->pSelect, "config") == 0 &&
                    ...
                    if (os_strcmp(pURL_Frame->pFilename, "reboot") == 0) {
                        ...
                        if (pParseBuffer != NULL) {
                            ...
                            SyslogSendString( "pParseBuffer==0\n" );
                            response_send(ptrespconn, false);
                        }
                    }
